...
public class TrayFlour2 : Mgr
{
    ...
    public bool AddChicken()
    {
        ...
        if (gameMgr.playData.tutoComplete1 == false && (tutoMgr.nowTuto == Tutorial.Tuto_3 || tutoMgr.nowTuto == Tutorial.Tuto_5) == false)
        {
            //해당 상태아니면 안되야됨
            return false;
        }
    ...
    public void ClickChickens(float v)
    {
        ...
        if (gameMgr.playData.tutoComplete1 == false && (tutoMgr.nowTuto == Tutorial.Tuto_4 || tutoMgr.nowTuto == Tutorial.Tuto_5) == false)
        {
            //해당 상태아니면 안되야됨
            return;
        }
        ...
        if(gameMgr.playData.tutoComplete1 == false && tutoMgr.nowTuto == Tutorial.Tuto_4 && IsComplete())
        {
            tutoObj2.PlayTuto();
        }
        ...
        if (IsMax())
        {
            ...
            if (gameMgr.playData.tutoComplete1 == false && allComplete)
            {
                //튜토리얼을 진행안한듯?
                //튜토리얼로 진입
