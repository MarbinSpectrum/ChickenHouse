...
public class Dove : GuestObj
{
    public override void OrderGuest(NoParaDel fun = null)
    {
        ...
        if (menuIdx == 0)
        {
            showStr = LanguageMgr.GetText("DOVE_ORDER_1");
            if (requireMenu.drink == Drink.Cola)
            {
                showStr += "\n";
                showStr += LanguageMgr.GetText("DOVE_SIDE_COLA");
            }
            if (requireMenu.sideMenu == SideMenu.Pickle)
            {
                showStr += "\n";
                showStr += LanguageMgr.GetText("DOVE_SIDE_PICKLE");
            }
        }
        else if (menuIdx == 1)
        {
            showStr = LanguageMgr.GetText("DOVE_ORDER_2");
            if (requireMenu.drink == Drink.Cola)
            {
                showStr += "\n";
                showStr += LanguageMgr.GetText("DOVE_SIDE_COLA");
        ...
        {
            showStr = LanguageMgr.GetText("DOVE_ORDER_3");
            if (requireMenu.drink == Drink.Cola)
            {
                showStr += "\n";
                showStr += LanguageMgr.GetText("DOVE_SIDE_COLA");
    ...
    {
        string showStr = LanguageMgr.GetText("DOVE_HAPPY");
    ...
    {
        string showStr = LanguageMgr.GetText("DOVE_THANK_YOU");
    ...
    {
        string showStr = LanguageMgr.GetText("DOVE_ANGRY");
